 ...
 * connection pool's growth lock whatever happens to the connections (Cipher R6). The pool's queue
 * is bounded and a timed-out call is cancelled and removed from it (Cipher C7): while Redis is
 * slow, calls fail fast instead of piling up on an unbounded queue that outlives the outage.
 * Closing the store shuts the pool down (Cipher C8).
 */
public final class JedisBudgetStore implements BudgetStore, AutoCloseable {
  ...
  /** Shuts the platform-thread pool down, if this store has one (Cipher C8). No-op otherwise. */
  @Override
  public void close() {
    if (executor != null) {
