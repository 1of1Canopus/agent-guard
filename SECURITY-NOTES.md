...
## Review status
the security review's adversarial pass (`docs/SECURITY-REVIEW-feat-agent-guard-core.md`): H1–H3 and M1–M7 fixed on the branch,
...
- [x] Fuzz `ArgumentRedactor` with adversarial JSON (done by the security review; gaps tracked as L5).
...
- [x] Threat model the webhook (done by the security review; signature tracked as L6).
