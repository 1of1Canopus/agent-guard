...
#   (Apache-2.0) (GPL-3.0) syn-dual (the security review.synthetic:syn-dual:1.0 - no url defined)
#
...
run_self_test() {
  ...
  # tokens that must be DENIED (the security review's N2 repro table, plus the original SPDX spellings).
  local -a deny_cases=(
    "GPL-3.0"
    "GPLv3"
  ...
  # scan. Case 1 is the security review's URL-forgery repro: a dependency's own <url> contains a balanced
  ...
  # G1: the security review's forward-forgery repro - a dependency's own <url> closes its coordinate
  # group early and opens a fresh, allowlisted one after it. The line now has TWO
  # coordinate-shaped top-level groups (the real the security review.synth:evil-c:1.0 and the trailing
