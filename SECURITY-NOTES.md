...
- **Residual (Cipher R4):** a role that owns the tables can disable the append-only and anchor triggers and rewrite
  ...
  periodically. The anchor's `agentguard_audit_anchor_no_delete`/`_no_truncate` triggers (Cipher F3(a)) mean that
  role cannot lose the anchor row even by accident.
- **Keyed-from-birth (design change, QUESTIONS.md #20, superseding the earlier per-row-version design and Cipher
...
- **Key rotation, from row 1 (amendment, Dollar, after Cipher's design review):** the key id
...
- **`NO_ANCHOR` unconditionally, keyed or unkeyed (amendment, widening Cipher F3(a)):** the first version of this
  ...
  downgrade to `ag1`/GENESIS from a genuinely keyed trail — Cipher's original V2 repro) cannot also flip `keyed`,
...
- **`InMemoryAuditSink` has no external anchor (Cipher H3, doc-only):** its `anchor()` is derived from the very
...
- **Test:** `AuditChainVerifierTest.a_key_given_with_no_anchor_reports_no_anchor_never_intact`,
  ...
  `CipherProbeReverifyTest.probe_a_fully_downgraded_trail_verifies_as_broken_not_intact` (Cipher's original
...
full (including Cipher's original whole-trail repro) more simply than the sequence-position design did, and closes
...
concurrent callers now fails closed (`AG-GUARD-001`) instead of queueing unboundedly, by design (Cipher C7) — size
the pool at or above real peak concurrency, and size `platform-thread-count`/`platform-thread-queue-size` (Cipher
